...
def stunting():
    ...
    ssa_value_2020 = str(int(df.loc[(df.iso_code == 'SSA') & (df.date == 2020), 'value'].iloc[-1]))
    world_value = str(int(df.loc[df.iso_code == 'WLD', 'value'].iloc[-1]))

    return {'stunting_ssa_value': ssa_value,
...
def ipc_dynamic(ipc):
    ...
    return {'phase3plus_world_value': round((sum(ipc.phase_3plus)) / 1000000, 0),
            'phase5_world_millions': round((sum(ipc.phase_5)) / 1000000, 0)}


def insufficient_food_dynamic():
    """ """
    ...
    return {'insufficient_food_latest_value': str(str(round(latest_value / 1000000, 2))),
            'insufficient_food_month_change': str(round(change, 2)),
            'insufficient_food_date': latest_date.strftime("%d %b %Y")
            }


def update_hunger_dynamic_text():
    """ """
